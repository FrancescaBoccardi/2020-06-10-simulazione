...
public class ImdbDAO {
	...
	public List<Arco> getArchi(String genre, Map<Integer,Actor> idMap){
		String sql = "SELECT r1.actor_id AS id1, r2.actor_id AS id2, COUNT(*) AS peso "
				...
				+ "WHERE r1.actor_id>r2.actor_id AND r1.movie_id=mg.movie_id AND r2.movie_id=r1.movie_id AND mg.genre=? "
		...
		try {
			...
			while (res.next()) {
				...
				Arco a = new Arco(idMap.get(res.getInt("id1")), idMap.get(res.getInt("id2")), res.getInt("peso"));
				result.add(a);
			}
			conn.close();
			return result;
